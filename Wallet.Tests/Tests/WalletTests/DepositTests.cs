...
public class DepositTests(TestFactory factory) : ApiTestBase(factory)
{
    [Fact]
    public async Task Deposit_ShouldIncreaseBalance()
    {
        ...
        var deposit = await Helper.Deposit(walletId: user.WalletId, amount: 100);
        deposit.WalletId.Should().Be(user.WalletId);
        deposit.Amount.Should().Be(100);
        deposit.Balance.Should().Be(100);
        deposit.CreatedAt.Should().BeCloseTo(DateTime.UtcNow, TimeSpan.FromSeconds(5));
        deposit.TransactionId.Should().BeGreaterThan(0);
        deposit.Type.Should().Be((int)TransactionType.Deposit);

        var balance = await Helper.GetBalance(user.WalletId);
    ...
    public async Task Deposit_InvalidAmount_ShouldFail()
    {
        ...
        var res = await Helper.DepositRequest(walletId: user.WalletId, amount: -100);
    ...
    [Fact]
    public async Task Deposit_ToAnotherUserWallet_ShouldFail()
    {
        ...
        var res = await Helper.DepositRequest(walletId: u2.WalletId, amount: 100);
        res.StatusCode.Should().Be(HttpStatusCode.BadRequest);
